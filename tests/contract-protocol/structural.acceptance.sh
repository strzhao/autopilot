...
# [2026-09-07 v3.63.0 语义化适配] contract-checker 独立 agent 已并入 qa-reviewer Section D：
#   C2 重指向 qa-reviewer-prompt.md Section D 结构契约；C3 重指向 SKILL.md Section D 引用；
#   functional-meta.acceptance.sh 随被测对象（contract-checker-prompt.md）删除而删除。
set -uo pipefail
...
# ── C2: qa-reviewer-prompt.md 存在 + 含 Section D 契约符合性结构 ─────────────
# 设计（v3.63.0 适配）：contract-checker 并入 qa-reviewer Section D——
# qa-reviewer-prompt.md 含 `## Section D: 契约符合性` 输出契约 + severity 枚举
CHECKER_PROMPT="$REFS_DIR/qa-reviewer-prompt.md"
[[ -f "$CHECKER_PROMPT" ]] \
    || fail_hard "2" "qa-reviewer-prompt.md 不存在: ${CHECKER_PROMPT}（前置条件失败，终止测试）"

if grep -qF '## Section D: 契约符合性' "$CHECKER_PROMPT" && grep -qE 'severity=high\|medium\|low' "$CHECKER_PROMPT"; then
    pass "2" "qa-reviewer-prompt.md 存在 + 含 '## Section D: 契约符合性' + severity 枚举（契约校验已并入 Section D）"
else
    echo "  --- grep 'Section D|severity' ---"
    grep -nE "Section D|severity" "$CHECKER_PROMPT" || true
    echo "  ---"
    fail "2" "qa-reviewer-prompt.md 缺少 '## Section D: 契约符合性' 输出契约或 severity 枚举（契约校验并入 Section D 的结构锚点）"
fi

# ── C3: SKILL.md 含「Section D」引用（smoke 行 + 结果判定）────────────────────
# 设计（v3.63.0 适配）：步骤 2.5 contract-checker 串行点已删，契约校验由 qa-reviewer
# Section D 承担；SKILL.md smoke 行与 Wave 2 填入项须引用 Section D（≥2 处）
SKILL_MD="$SKILL_DIR/SKILL.md"
[[ -f "$SKILL_MD" ]] \
    || fail_hard "3" "SKILL.md 不存在: ${SKILL_MD}（前置条件失败，终止测试）"

c3_hit=$(grep -cF "Section D" "$SKILL_MD" || true)
if [[ "$c3_hit" -ge 2 ]]; then
    pass "3" "SKILL.md 含「Section D」引用 $c3_hit 处 ≥ 2（smoke 行 + Wave 2 填入项均锚定 Section D）"
else
    echo "  命中数: $c3_hit / 2"
    echo "  --- grep 'Section D' ---"
    grep -nF "Section D" "$SKILL_MD" || echo "  (无命中)"
    echo "  ---"
    fail "3" "SKILL.md 「Section D」引用仅 $c3_hit 处 < 2（smoke 路径契约校验保留 + Wave 2 契约章节填入须双锚定）"
fi
...
# ── C11: 4 处版本号字符串全部对齐 plugin.json（动态读取，治硬编码过时假红） ────────────
# 设计：plugin.json / marketplace.json / 根 CLAUDE.md / plugins/autopilot/README.md 四处同步
# [2026-09-07] TARGET_VERSION 从硬编码 3.24.0 改为动态读 plugin.json——版本越过 3.24.0 后必假红；
# 顺带修复 `$var（` 全角括号变量名陷阱（[2026-05-30]，set -u 下崩溃），全部加 braces。
PLUGIN_JSON="$REPO_ROOT/plugins/autopilot/.claude-plugin/plugin.json"
TARGET_VERSION=$(grep -oE '"version":[[:space:]]*"[0-9]+\.[0-9]+\.[0-9]+"' "$PLUGIN_JSON" | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+')
[[ -n "$TARGET_VERSION" ]] || { echo "[FAIL] C11: 无法从 plugin.json 读取版本号" >&2; exit 1; }
