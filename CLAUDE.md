...
| 插件 | 版本 | 类型 | 一句话 |
|------|------|------|--------|
| [autopilot](plugins/autopilot/) | v3.37.1 | Skill + Hook | AI 自动驾驶工程套件：全流程闭环（QA ⚠️ 复盘）、AI 自适应 fast/standard、智能提交、工程诊断、worktree 自动初始化（选择性 symlink）+ brainstorm 独立 skill + plan-review HTML 左侧 TOC 导航 + `.autopilot/` 二级分层 knowledge/+runtime/（三层防御）+ **QA Tier 5 量化指标门禁** + **state.md 内容区瘦身 — 第一性原理删除变更日志铁律 + 元数据范围控制补丁，主上下文不再被无关元数据 Edit 充斥** + Tier 1 AI-First 反过拟合减法（plan-reviewer 去伪精度分数线 / qa-reviewer 正则转语义 / Explore 计数自适应 / 防合理化去重） |
