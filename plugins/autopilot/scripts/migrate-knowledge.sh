...
NEW_PATH="$PROJECT_ROOT/.autopilot/knowledge"
...
if [[ -L "$OLD_PATH" ]]; then
    ...
    # 检查主仓库是否已完成迁移
    if [[ -z "$MAIN_REPO" || ! -d "$MAIN_REPO/.autopilot/knowledge" ]]; then
    ...
    # NEW 已存在
    if [[ -e "$NEW_PATH" ]]; then
        # 已有 .autopilot/knowledge，只清理旧符号链接
        rm "$OLD_PATH"
        echo "✅ 已移除旧符号链接 .claude/knowledge"
    else
        # 创建 .autopilot/knowledge 符号链接指向主仓库
        mkdir -p "$PROJECT_ROOT/.autopilot"
        ln -s "$MAIN_REPO/.autopilot/knowledge" "$NEW_PATH"
        rm "$OLD_PATH"
        echo "✅ worktree 符号链接已更新: .autopilot/knowledge → $MAIN_REPO/.autopilot/knowledge"
    fi
    exit 0
fi
...
if [[ -d "$OLD_PATH" ]]; then
    ...
    if [[ -d "$NEW_PATH" ]]; then
        ...
        echo "✅ 已将 .claude/knowledge/ 内容合并到 .autopilot/knowledge/（跳过已存在文件）"
    else
        # NEW 不存在 → 直接移动
        mv "$OLD_PATH" "$NEW_PATH"
        echo "✅ 已将 .claude/knowledge/ 迁移到 .autopilot/knowledge/"
    fi
