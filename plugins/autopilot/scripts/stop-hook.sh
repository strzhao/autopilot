...
if [[ "$PHASE" == "design" ]]; then
    if [[ "$AUTO_APPROVE" == "true" ]]; then
        ...
        PROMPT="读取 ${STATE_FILE} 状态文件获取目标描述, fast_mode=true: 砍所有 plan-review 类节点（红蓝对抗 / QA Wave 1+1.5 是核心，保留不变）. design 阶段: 跳过 brainstorm Q&A，只用 1 个 Explore agent 探索代码，不启动 scenario-generator / plan-reviewer Agent — 设计文档写入状态文件后**直接设 phase: implement**（跳过 AskUserQuestion 审批，fast 信任 AI 判断；html_review=true 时改走步骤 4c HTML 评审）. implement 阶段: blue/red-team 双 Agent 照常启动. 详见 SKILL.md Fast Mode 快速路径章节. 按照 autopilot skill 的 Phase: design 指引执行."
    else
