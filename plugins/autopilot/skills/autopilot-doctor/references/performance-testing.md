...
下列子报告在 doctor 主报告生成时，仅当对应维度评分 ≤ 8 且非 N/A 时按需展开。SKILL.md 报告模板内引用此章节。
...
| P3: Bundle Size | ✅/❌/N/A | 工具 + 配置 + 阈值 + 构建产物体积 |
...
## Top 3 与 Quick Fixes 报告模板

doctor 主报告的改进建议段模板，按投资回报率（影响/工作量）排序。

### Top 3 改进建议

每项含：问题（一句话短板）/ 影响（解锁哪些 autopilot 功能）/ 解决方案（1-3 步）/ Quick Fix（一行命令，如有）/ 预估耗时。

### Quick Fixes

可立即执行的一行命令（复制粘贴即用），按优先级列出 3-5 条。
