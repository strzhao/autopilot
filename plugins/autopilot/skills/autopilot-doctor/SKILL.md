...
1. 检测传入参数（是否 `--fix`）
2. 技术栈检测（前置步骤）
3. Wave 1：并行命令检测（Dim 1-4, 8-9, 11-12）
4. Wave 2：串行 AI 判断（Dim 5-7, 10, 12）
5. 计算加权总分 → 生成报告
...
ls .autopilot/index.md .autopilot/decisions.md .autopilot/patterns.md 2>/dev/null; \
ls .autopilot/domains/ 2>/dev/null; \
# 文件大小检测
wc -l .autopilot/decisions.md .autopilot/patterns.md 2>/dev/null; \
find .autopilot/domains/ -name "*.md" -exec wc -l {} + 2>/dev/null; \
# 索引一致性：index.md 条目数
grep -c "^\- \[" .autopilot/index.md 2>/dev/null || echo "0"; \
# 实际内容条目数（### [YYYY-MM-DD] 标题数）
grep -rh "^### \[" .autopilot/decisions.md .autopilot/patterns.md .autopilot/domains/ 2>/dev/null | wc -l; \
# 元信息完整性：抽样检查（取前 30 行）
head -30 .autopilot/decisions.md 2>/dev/null; \
head -30 .autopilot/patterns.md 2>/dev/null
```
...
3. **文件大小健康度**：全局文件（decisions.md/patterns.md）≤100 行、领域文件（domains/*.md）≤150 行
4. **索引一致性**：`index.md` 中的 `- [日期]` 条目数 vs 内容文件中实际 H3 三级标题（`[日期]` 开头）数量，偏差 ≤1 为正常
