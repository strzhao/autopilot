...
1. 检测 `--fix` 参数 → 2. Step 0 技术栈检测 → 3. Wave 1 并行命令检测（Dim 1-4, 8-9, 11-13 客观部分）→ 4. Wave 2 串行 AI 判断（Dim 5-7, 10, 12-13 语义部分）→ 5. 加权总分 → 报告 → 6. 保存 `.autopilot/runtime/doctor-report.md` → 7. `--fix` 模式针对低分维度提供修复方案

---

## Step 0: 技术栈检测

**执行方式**：调 `detect_tech_stack()`（lib.sh SSOT），返回 JSON `{node,swift,go,python,rust,java,primary}`。
...
- **测试模板可复制性**：现有测试文件是否有清晰模式可供 AI 参考
- **红队测试可行性**：项目是否有足够接口定义让红队仅凭设计文档就能写测试
- **AI 友好的 script**：package.json scripts 是否语义清晰、可独立运行
- **API Schema 可发现性**：有 OpenAPI/GraphQL schema 则红队可写契约测试
- **Mock 基础设施**：有 msw/nock + fixtures 则红队/蓝队写测试效率更高
- **可测试性设计**：抽样 1-2 个核心模块，检查依赖是否通过参数注入而非硬 import
...
1. **过拟合密度**：扫描 `Lesson`/`Choice` 字段行（**跳过** `Evidence`/`Background`/`Trade-offs` 行），检测其中包含版本号、文件路径、行号、具体计数的条目数量
2. **重复/冗余主题**：检测 tags 重叠 ≥3 且语义相似的条目对
3. **文件大小健康度**：全局文件（knowledge/decisions.md / knowledge/patterns.md）≤100 行、领域文件（knowledge/domains/*.md）≤150 行
4. **索引一致性**：`knowledge/index.md` 中的 `- [日期]` 条目数 vs 内容文件中实际 H3 三级标题（`[日期]` 开头）数量，偏差 ≤1 为正常
5. **元信息完整性**：抽样 entry 是否有 `[日期]` + `<!-- tags: ... -->` + 必需字段（Lesson 或 Choice）
6. **文件分类正确性**（v3.35+ 三层防御 Layer 3，C3 契约）：
...
- **过拟合建议**：`条目标题` → 当前 Lesson 一句话 → 建议泛化改写一句话
- **重复合并建议**：`条目 A` + `条目 B` → 合并方向（谁的 Lesson 更抽象则保留并扩充 Evidence）
- **超阈值建议**：`文件名` 当前 X 行 → 建议拆分到 `domains/{domain}.md`
...
各维度评分 ≤ 8 且非 N/A 时，按 [references/performance-testing.md](references/performance-testing.md) §doctor 报告详情子报告模板 展开对应子报告（Dim 1/11/12）。Dim 13 详情：非 PASS 维度引用 [references/ai-observability-principles.md](references/ai-observability-principles.md) 对应 `DIM-13-XX` 核心原则段驱动 AI 自主调研修复。
...
|----------------|------|----------|------|
| 红队验收测试 / Tier 0: 红队 QA | ✅/⚠️/❌ | Dim 1 | 需要测试框架；无框架时降级为文本检查清单 |
| Tier 1: 类型检查 / Lint / 单元测试 / 构建验证 | ✅/⚠️/❌ | Dim 2/3/1/4 | 分别需要 TypeScript/mypy、ESLint/Biome、测试框架、build 命令 |
...
| 安全审查（code-quality-reviewer） | ✅/⚠️/❌ | Dim 9 | 需要 input validation 库 + 安全基线 |
...
## Top 3 改进建议 + Quick Fixes

按投资回报率排序，模板见 [references/performance-testing.md](references/performance-testing.md) §Top 3 与 Quick Fixes 报告模板（每项含问题/影响/解决方案/Quick Fix/预估耗时；Quick Fixes 按优先级列 3-5 条可立即执行的一行命令）。
...
当用户使用 `--fix` 时，在完成诊断报告后，针对每个分数 ≤ 6 的维度：向用户展示改动（文件名 + 内容摘要）→ **AskUserQuestion** 确认 → 生成/修复配置文件 → 重新运行对应维度检查验证修复效果。

### 常见修复方案

| 维度 | 修复动作 |
|------|----------|
| 测试基础设施 (L1/L2/L3) | L1：安装测试框架 + 生成配置 + 创建示例测试；L2：创建 API route 测试示例（见下方 L2 修复详情）；L3：安装 Playwright + 生成配置 + 创建 E2E 示例（见下方 L3 修复详情） |
...
1. **检测 API 框架**：Next.js App Router（`app/api/` 存在 → 直接 import handler）| Express/Fastify（`router.get/app.get` 模式 → 推荐安装 supertest）
2. **扫描现有测试模板**：搜索 `*.acceptance.test.*` 或 `*.integration.test.*` 文件，取第一个文件的 import/mock 模式作参考，无则用通用骨架
3. **生成文件**：创建 `__tests__/api/` 目录 + 1 个示例 API route test（基于项目最简单的 API 路由，含 auth mock、db mock、成功/失败两用例）
...
2. **生成 `playwright.config.ts`**：自动检测 dev server 命令（package.json `scripts.dev`）+ 端口（dev 命令参数如 `--port 4000`）+ 配置 `webServer` 自动启动 dev server + 只配置 chromium 项目
3. **生成 `e2e/` 目录 + 示例 spec**：1 个冒烟测试（访问首页 → 验证页面标题 → 截图）
4. **更新已有配置**：`vitest.config.ts` 添加 `exclude: ['e2e/**']`（如存在）+ `package.json` 添加 `"test:e2e": "playwright test"` + `.gitignore` 添加 `test-results/`、`playwright-report/`、`blob-report/`
