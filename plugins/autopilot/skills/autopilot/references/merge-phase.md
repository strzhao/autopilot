...
进入 merge 阶段后，立即回顾本次全流程产出，提取值得持久化的知识（时间限制 2 分钟，宁可少写高质量条目不要穷举）。写入 `.autopilot/knowledge/` 后设 `knowledge_extracted: true/skipped`，**不单独 commit**——普通模式下由步骤 3 commit Agent 的 `git add -A` 一并提交；worktree 模式见下方「提交路由」。
...
### 触发条件（全部满足才执行）
1. 项目根目录存在 `.github/workflows/*.yml`
2. `gh` CLI 可用
3. 远端能查到本次 commit 触发的 CI run

### 执行流程
1. 找到本次 commit 对应的 run id
2. `gh run watch <run-id> --exit-status`，超时 600s
3. CI 通过 → 继续步骤 5
4. CI 失败 → 设置 frontmatter `phase: "auto-fix"` 和 `qa_scope: "selective"`

### 降级
任何一项不满足即跳过，不阻塞。

## 5. Auto-Chain 评估（brief 模式专用）
...
## 7. 清理
- 更新 frontmatter：`phase: "done"`，同时确认 `gate: ""` 清空
- Stop hook 检测到 done 后会自动清理状态文件并发送完成通知
- 如果设置了 `next_task`，stop-hook 会自动创建下一个任务的状态文件并继续循环

## 附：worktree 提交路由

worktree 中 `.autopilot/knowledge/` 是 symlink 指向主仓库，commit Agent 在 worktree 跑 `git add -A` 不会包含知识库。统一兜底：

1. commit Agent 之后，通过 realpath `.autopilot/knowledge/` 的 symlink 锚点定位主仓库
2. 在主仓库执行 `git status --porcelain .autopilot/`
3. 非空才 `git add .autopilot/ && git commit -m "docs(knowledge): <brief>"`

普通模式下主仓库=当前仓库，commit Agent 已提交，git status 干净 → 跳过。
