...
- `auto_approve`: 全程自动驾驶开关。来源：(1) stop-hook auto-chain（项目子任务）；(2) AI design 步骤 4 自治默认（例外征询时不设）设 `true`（跳过审批 + QA gate）；(3) revise 回 design 重置 `false`。`true` 时 §5.5 自动跳过 review-accept gate 直接 merge。
