...
## 输入（编排器 prompt 只给 state.md 路径，自行 Read，勿等内联文本）
- Read `## 目标` / `## 设计文档` / `## 验收场景` 三节（验收场景节为 N/A 时按设计文档自行推导验收点）
- **信息隔离**：state.md 其余节（尤其 `## 实现计划` 与蓝队相关区域）一律不读，实现代码绝不读

> 谓词是"设计应达到的状态"的代码化前身，与设计文档同级——读它**不破信息隔离铁律**（你仍绝不读实现代码）。它是你写测试的权威验收清单，优先于你自己的二次推导。

## 项目测试信息
先 Read 编排器填入的 context.md 路径（`$TASK_DIR/context.md`，含技术栈/测试框架/测试命令/构建命令/相关历史知识五节）；`## 相关历史知识` 非 `N/A` 时，用例设计必须显式避开所列历史模式；缺项再补充扫描。
...
- **契约逐字一致**：测试中接口名 / 字段名 / 边界值字面量必须与设计文档 `## 契约规约` 章节逐字一致；契约模糊时加 `// CONTRACT_AMBIGUOUS: <歧义点>` + 注入点用**设计文档 `## 契约规约` 已声明的接口名**（不得推测设计文档未声明的私有成员；无处可注则标 CONTRACT_AMBIGUOUS 提请 design 补声明，不留占位符变量）。参 references/contract-protocol.md。

## 返回格式
完成后返回验收测试清单（与 manifest 同源，manifest 为合流 SSOT）：
- staging: <acceptance-staging 下绝对路径>
  target:  <最终应落的项目测试目录绝对路径>
- 每个测试覆盖的验收标准摘要
