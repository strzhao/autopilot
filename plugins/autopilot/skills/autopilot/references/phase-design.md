...
## 关键规则
**进入 design 阶段后，依次执行：知识上下文加载（如 `.claude/knowledge/` 存在）→ 需求澄清（如触发）→ 调用 `EnterPlanMode` 工具。** 知识加载不超过 15 秒。需求澄清仅在目标不够明朗时触发（见步骤 0.5）。所有代码探索在 Plan Mode 内完成。
...
知识加载完成后，评估目标描述的清晰度，决定是否在进入 Plan Mode 前与用户澄清关键歧义。

**⚠️ 仅在 `iteration=1` 时执行此步骤。** stop-hook 循环回来时跳过，避免重复提问。

#### 触发信号（命中任一即触发）

| 信号 | 示例 |
|------|------|
| 目标描述 < 20 字且无具体技术指向 | "优化性能"、"改进体验" |
| 涉及多种实现路径且用户未指定偏好 | "加个缓存"（Redis? 内存? 文件?） |
| 跨 3+ 独立模块或子系统 | "重构认证 + 支付 + 通知" |
| 引用外部系统但未说明集成方式 | "接入 Slack 通知" |
| 包含模糊量词或主观描述 | "更好的"、"更快的"、"合理的" |

#### 不触发（直接跳到步骤 1）

- 目标清晰且具体（如 "在 X 文件的 Y 函数中修复 Z bug"）
- 用户已指定技术方案（如 "用 Redis 做缓存"）
- 变更范围明确且局部（如 "给 API 加一个字段"）
- 用户引用了具体代码位置或文件

#### 触发后的行为

1. 使用 `AskUserQuestion` 工具提出 **1 个**聚焦问题（复杂场景最多 2 个，不可更多）
2. 问题**优先使用多选格式**（2-4 个选项），带推荐标记（`(Recommended)` 后缀）
3. 聚焦于影响设计方向的关键歧义，不问实现细节
4. 问题分类参考：范围确认 / 技术路径偏好 / 优先级排序
5. 收到回答后将澄清结果作为内部上下文，与知识上下文一并带入 Plan Mode

#### 降级

用户不回答或回答"不确定"时，不阻塞流程，直接进入步骤 1。步骤 0.5 是辅助性的，不应成为阻断点。

### 步骤 1. 进入 Plan Mode
- 从状态文件读取目标描述
- **调用 `EnterPlanMode` 工具** —— 除知识加载和需求澄清外，这是 design 阶段的第一个工具调用
