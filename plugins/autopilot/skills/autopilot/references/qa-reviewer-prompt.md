...
- **Section D: 契约符合性审查** — 字面比对设计文档 `## 契约规约` 与实现的接口形状（无契约要求时输出 N/A）

合并的目的是节省 sub-agent cold start 成本（此前 design-reviewer + code-quality-reviewer 两个 Agent 各自冷启动 + 各自 Read 同一批变更文件，合并后省 ~1M token / run）。四类审查的关注点互补，不重叠：A 关注「应该实现什么 vs 实际实现什么」，B 关注「实现得是否安全/健壮」，C 关注「红队测试本身是否有真实断言力」，D 关注「接口形状 vs 契约字面是否一致」。
...
2. **范围检查**：
   ...
   - 偏离（实现方式与设计不一致）
3. **Wave 1 失败关联**：失败是否暴露了设计偏离或需求理解偏差
4. **验证层级兑现**：设计文档 `## 验证方案` 声明的测试层级（E2E/API 集成/输入验证）是否都有对应的红队测试文件产出？如有缺失 → 标记为 Important（置信度 85），不标记为 BLOCKER
5. **Tier 1.5 ⚠️ 独立审查**：对编排器提供的「Tier 1.5 ⚠️/❌ 场景原始命令输出」，独立读取原始 stdout/stderr 并判断属于「测试环境/工具配置问题」还是「功能问题」。如果是功能问题（如真实用户路径报错、e2e 超时反映功能不可用、断言失败）但被编排器标为 ⚠️ → 标记为 BLOCKER（置信度 ≥90），在 Section A「缺失项」中追加 `[BLOCKER] Tier 1.5 ⚠️ 复盘错误: <场景> 实际为功能问题`
...
**核心原则：字面比对** — 复用「共同准备」已读的变更文件全文，比对设计文档 `## 契约规约` 与实现的接口形状，不评估行为、不跑测试。比对维度 5 类：`field_name`（字段名逐字，区分大小写/驼峰/下划线）/ `boundary`（边界值运算符，如 `≤` vs `<`）/ `error_code`（错误码枚举名）/ `route`（路由路径字面量）/ `signature`（参数名与返回值字段名）。severity 枚举：`high`（字面量完全不同，如 `count` vs `memberCount`）/ `medium`（契约模糊无法确定，或等价表达如 `<=` vs `≤`）/ `low`（风格级偏差不影响调用方）；severity=high 条目 ≥1 → 计入 Critical（谓词闸门「0 Critical」据此拦截）。
...
- **谓词充分性反查**（基于共同准备已读的代码改动 `git diff` + 变更文件全文）：反向枚举本次改动触及的风险面 —— 新增分支与错误处理路径 / 对外部输入（用户输入、legacy 数据、第三方返回）的假设 / 向后兼容·降级契约（字段 optional vs required、旧格式）/ 同一行为的多个执行入口 / 并发·异步·状态机路径 / **文档型交付物声明的可执行工作流**（README/方案/设计文档里写了「运行 X / 调用 Y / 访问 Z」的链路——纯文档改动也有执行面）。**语义判断哪些与本次改动真正相关**，逐个反查 `## 验收场景` 是否有谓词覆盖。未覆盖的风险面列入下表（Important，置信度 80+，**非 BLOCKER** —— 漏覆盖 ≠ 实现错）。
...
- **执行面一致性反查**（防虚标 verified + 清单漏列）：据 Tier 1.5 三元组 + 证据链反查 frontmatter `e2e_status` 分级是否成立——存在真实驱动跳过格 / 关键链路仅 mock 覆盖 / 首跑推迟却标 `verified` → 不一致列 Important（置信度 85+，注明反查依据 file:line 或 artifact 路径）。**另做两步执行面核验**：① 对照设计文档声明交付物 + 新增文件清单枚举本次任务的可执行链路全集，反查验收决策卡「### 端到端真实验证结论」执行面清单是否漏列（清单漏列设计声明的可执行链路 → 漏报未执行，列 Important）；② 三方一致核验——清单（存在未执行核心链路）∧ 字段（`unexecuted_core_paths=0` ∨ `e2e_status=verified`）任一矛盾（清单点名了未执行核心链路却 `unexecuted_core_paths=0` 或 `e2e_status=verified`）→ **Critical**（清单是硬结构无歧义）。分级语义见 `references/state-file-guide.md`。

### Section D — 契约符合性

`## Section D: 契约符合性` 标题下内容二选一：`N/A`（contract_required=false 或设计文档无 `## 契约规约` 时，恰为 N/A）或 mismatch 表（每行含 severity=high|medium|low）：

| # | 类型 | 契约期望 | 实现实际 | severity |
|---|------|----------|----------|----------|
| 1 | field_name | `memberCount` | `count` (src/burst.ts:42) | severity=high |

severity=high 条目数 ≥1 ⟹ 报告 Critical 数 ≥1（计入谓词闸门）。
