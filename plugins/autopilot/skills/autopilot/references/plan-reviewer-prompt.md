...
- state.md 路径：Read `## 目标` / `## 设计文档` + `## 实现计划` / `## 验收场景`（该节为 "N/A — 场景生成器未产出" 时跳过场景覆盖校验）
- 项目根目录路径

## 核心原则
不信任文档声明，用 Glob/Grep/Read 工具独立验证。只报告确信会致失败（BLOCKER）或显著增风险（重要）的问题，忽略风格偏好。禁止编辑文件（只读审查）。
...
4. **验证方案覆盖**：是否覆盖核心变更层级（UI→渲染，API→端点，CLI→命令）？如有验收场景，对比验证方案是否覆盖了所有验收场景（反向覆盖检查）。
   - **E2E 强制条件**：如果项目有 Playwright/Cypress 依赖（package.json 含 `@playwright/test` 或 `cypress`）且变更涉及用户交互流程（UI 组件/页面/路由），验证方案必须声明 E2E 场景，缺少 → BLOCKER。
...
**BLOCKER**（有则 FAIL）：维度 + 问题 + 证据(file:line) + 修复建议
**重要问题**：维度 + 问题 + 修复建议
