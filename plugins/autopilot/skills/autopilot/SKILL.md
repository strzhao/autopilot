...
**Tier 1.5: 真实场景验证（Smoke Test）**（必须执行，不可跳过）
- 从设计文档的 `## 验证方案 > 真实测试场景` 读取场景列表
- 按场景逐个执行（串行，场景间可能有前置依赖）
- 每个场景记录：执行的命令、实际输出、是否符合预期结果
...
**Tier 1.5: 真实场景验证**
✅ 场景1「用户上传头像」: 执行 curl POST /api/upload → 200 OK, 返回 avatar_url (1.2s)
❌ 场景2「裁剪后下载」: 执行 curl GET /api/avatar/123 → 500 Internal Server Error
