...
- **design**：知识加载 → **1 个** Explore agent → 设计文档写入状态文件 → 按 `references/plan-reviewer-prompt.md` 全部维度**自审**（不启动 scenario-generator / plan-reviewer Agent，不做 brainstorm Q&A）→ 自审通过后**直接 `phase: "implement"`**（跳过 AskUserQuestion 审批，fast 信任 AI 判断；自审失败修正一次，仍失败才回退 AskUserQuestion 交用户）
- **implement**：blue-team / red-team 双 Agent 保留不变，但**跳过 contract-checker Agent**（步骤 2.5 在 fast_mode=true 时直接进入 qa）
- **qa**：qa_scope=smoke（详见 Phase: qa 前置选择性重跑判断），不启动 qa-reviewer Agent
...
**触发条件**：读取状态文件 frontmatter `contract_required` 和 `fast_mode` 字段。

- `fast_mode: true` → **跳过本步骤**（fast 砍 plan-review 类节点，红队验收测试仍覆盖契约违反）
- `contract_required` 字段缺失或值为 `false` → **跳过本步骤**，直接进入 Phase: qa
- `contract_required: true` 且非 fast → 启动 contract-checker Agent 执行字面校验
