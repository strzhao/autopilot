...
4. 其他（空或 `"standard"`）→ 走标准模式：先执行知识上下文加载（如 `.autopilot/` 存在），然后立即调用 `EnterPlanMode` 工具。知识加载不超过 15 秒。所有的代码探索工作都应该在 Plan Mode 内完成。
...
`fast_mode: true` 时砍掉 scenario-generator 和 plan-reviewer 两个 Agent：知识加载 → `EnterPlanMode` → **1 个** Explore agent（**不启动 scenario-generator**）→ 编排器按 `references/plan-reviewer-prompt.md` 中 6 维度（需求完整性/技术可行性/任务分解/验证方案/风险/范围控制）**自审**（**不启动 plan-reviewer Agent**）→ `ExitPlanMode`。自审失败可在 plan file 中修正后重审一次，仍失败则标注问题继续 ExitPlanMode 交用户判断。
...
仍在 Plan Mode 中，将项目级内容（Context / 整体架构设计 / 任务 DAG 概览 / 跨任务设计约束 / Handoff 策略）写入计划文件，替代标准单任务 plan 模板。完整 markdown 模板参见 [references/state-file-guide.md](references/state-file-guide.md)。完成后执行步骤 3（Plan 审查）和步骤 5（ExitPlanMode）。审批通过后走 [步骤 6b. 项目模式文件创建](#步骤-6b-项目模式文件创建)。
