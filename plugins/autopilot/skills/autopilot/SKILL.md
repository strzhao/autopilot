...
3. **重审控制**：最多 5 轮（初审 + 4 次重审）；**停点由 BLOCKER 决定而非轮数**——任一轮 0 BLOCKER 即 PASS 进步骤 4，**不得自行加轮去消除重要问题（80-89）**（重要问题不阻断，附设计文档末尾即可）；第 5 轮仍有 BLOCKER → 附未解决 BLOCKER 标注 `[审查未通过，交由用户判断]` 后继续步骤 4
