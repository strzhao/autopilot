...
> **Worktree 隔离**：在 git worktree 中运行时，active 指针和状态文件位于 `.autopilot/sessions/<worktree-name>/` 子目录下，与主仓库（`.autopilot/active`）和其他 worktree 完全隔离。知识文件（decisions.md、patterns.md）通过符号链接共享。
