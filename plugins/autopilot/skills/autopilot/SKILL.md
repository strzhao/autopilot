...
#### 5. 清理
- **⚠️ 必须先设 `knowledge_extracted`**：知识提取完成后设为 `"true"`（有新增）或 `"skipped"`（无新增），**然后再**设 `phase: "done"`。Stop hook 会检查此字段，未设置时会回滚到 merge 阶段强制执行知识提取。
- 更新 frontmatter：`knowledge_extracted: "true"` 或 `"skipped"`
- 更新 frontmatter：`phase: "done"`
- Stop hook 检测到 done + knowledge_extracted 已设置后会自动清理状态文件并发送完成通知
