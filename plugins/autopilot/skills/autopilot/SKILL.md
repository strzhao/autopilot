...
> **层级匹配原则**：场景必须覆盖核心变更层级。UI 组件变更 → 必须有渲染验证场景；API 变更 → 必须有端点调用场景；CLI 变更 → 必须有命令执行场景。仅有数据层验证的 UI 组件任务是不完整的。
...
1. **启动审查 Agent**：使用 Agent 工具启动 plan-reviewer，prompt 参考 `references/plan-reviewer-prompt.md` 模板，填入：
   - 目标描述（从状态文件 `## 目标` 复制）
...
##### 降级方案
- 如果 Plan Mode 内 Agent 工具不可用（调用失败），编排器自行在 Plan Mode 内逐维度执行简化版审查（不启动 sub-agent），重点检查：需求完整性、技术可行性、验证方案覆盖

##### 审查报告处理
- PASS 时：在 plan file 末尾追加一行 `> ✅ Plan 审查通过（{通过维度数}/6 维度通过）`
- FAIL 修复后 PASS：追加 `> ✅ Plan 审查通过（第 {N} 轮，修复了 {M} 个 BLOCKER）`
- 最终仍 FAIL：追加审查报告全文，标注交由用户判断
...
4. **红队产出检查门**
   红队 Agent 完成后，检查是否产出了测试文件：
   - **有测试文件** → 跳转到 [2. 合流](#2-合流--两个-agent-都完成后)
   - **红队未产出测试文件**（Agent 失败或项目无测试框架）→ 降级为验收检查清单（纯文本），写入 `## 红队验收测试` 区域，QA 阶段由 design-reviewer 逐项人工验证
...
- 红队 Agent 失败 → 降级为纯文本验收清单（参考 `3. 降级策略` 中的红队失败处理）

#### 审查后修改铁律（Post-Review Modification Rule）

**任何在外部审查/评分之后所做的代码修改，必须重新运行对应的验证。** 不允许"评分通过后优化一下就合入"。

| 场景 | 要求 |
|------|------|
| Gemini/外部 AI 评分后，基于建议修改代码 | 必须重新评分或至少重新运行 tsc + 相关测试 |
| 红队测试全部通过后，做"小优化" | 必须重新运行红队测试 |
| Code review 通过后，追加改动 | 必须重新运行受影响的 Tier |

> **教训**：little-bee 鼻字场景 Gemini 评分 96/100 PASS 后，基于 Gemini 建议将 spring 动画改为 3 关键帧，未重新验证直接合入。framer-motion 的 spring 只支持 2 关键帧，导致运行时崩溃。QA 全部"通过"后用户手动测试才发现。
...
| 核心变更类型 | 必须包含的场景类型 | 缺失时的处理 |
|-------------|-------------------|-------------|
| UI 组件（新增/修改 `.tsx`/`.vue` 渲染逻辑） | 启动 dev server + 浏览器/截图验证渲染 | QA 自行补充 UI 场景 |
| API 端点（新增/修改路由处理） | `curl`/`fetch` 调用真实端点 | QA 自行补充 API 场景 |
| CLI 工具/脚本 | 实际运行命令验证输出 | QA 自行补充命令场景 |
| 数据库变更（schema/migration/seed） | 查询验证数据状态 | QA 自行补充数据场景 |

> **教训**：little-bee 鼻字 NoseScene.tsx（461 行 UI 组件）的验证方案只有数据库查询和音频索引——全是数据层测试。QA Tier 1.5 执行这些场景全部通过，但组件实际渲染时 framer-motion 崩溃。如果验证方案包含一个 dev server + 页面访问场景，运行时错误在 QA 阶段就能被捕获。
...
| "场景 1 已验证了核心流程" | --help 验证"能启动"，hanzi list 验证"能用"。little-bee-cli 中 48 测试全通过但 4 个 bug 靠手动发现，原因就是只跑了 --help。 |
...
> **教训**：little-bee-cli 中 48 个测试全通过、QA 报告声称"无遗留风险"，但 4 个 bug 全靠用户手动发现。根因：设计了 3 个真实场景但只执行了 --help，跳过了需要 server 的场景。如果执行了 lb hanzi list，至少 3 个 bug 会被当场捕获。
...
4. 有值得记录的条目：
   ...
   f. 确定知识库 git 提交上下文（worktree 安全路由）：
      ...
      - **步骤 2**（非符号链接）：检查当前是否在 worktree 中（`.git` 是文件而非目录）
        - 是 worktree → 通过 `git rev-parse --git-common-dir` 解析主仓库根目录，将知识文件复制到主仓库后用 `git -C` 提交，并修复符号链接以便后续运行：
          ```bash
