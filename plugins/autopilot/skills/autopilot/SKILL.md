...
2. `plan_mode: "deep"` → 走 [Deep Design 模式](#deep-design-模式)（交互式探索 + Q&A）
3. `fast_mode: true` → 走 [Fast Mode 快速路径](#fast-mode-快速路径仅-fast_modetrue-时)（1 个 Explore agent，编排器自审）
4. 其他（空或 `"standard"`）→ 走标准模式工作流程

### Deep Design 模式

当 `plan_mode: "deep"` 时，执行交互式需求探索。此模式适用于需求不明确或需要深度讨论的场景。

单流程：知识加载 → Explore agent → 视觉伴侣征求 → 逐个问问题 → 方案选择 → 写设计文档到状态文件 → Plan Reviewer + Spec Reviewer → AskUserQuestion 审批

Q&A 结果写入 `$TASK_DIR/brainstorm.md`（`task_dir` 从 frontmatter 读取）。详细工作流参见 [deep-design-guide.md](references/deep-design-guide.md)。

### Fast Mode 快速路径（仅 fast_mode=true 时）

`fast_mode: true` 时砍掉 scenario-generator 和 plan-reviewer 两个 Agent：知识加载 → **1 个** Explore agent（**不启动 scenario-generator**）→ 将设计文档写入状态文件 → 编排器按 `references/plan-reviewer-prompt.md` 中 6 维度（需求完整性/技术可行性/任务分解/验证方案/风险/范围控制）**自审**（**不启动 plan-reviewer Agent**）→ AskUserQuestion 审批。自审失败可修正后重审一次，仍失败则标注问题交用户判断。
