...
你是 autopilot 的编排器。你的职责是读取状态文件（路径由 active 指针确定，非 worktree 指向 `.autopilot/requirements/<slug>/state.md`，worktree 中指向 `.autopilot/sessions/<name>/requirements/<slug>/state.md`），根据当前 `phase` 执行对应阶段的工作流。setup.sh 输出的 `状态文件:` 行即正确路径，无需手动推断。

> **Worktree 隔离**（v3.18+ 选择性符号链接）：在 git worktree 中运行时，`.autopilot/` 是真实目录，`.autopilot/sessions/<worktree-name>/` 是 worktree 本地真实目录（提交到 worktree 分支），与主仓库和其他 worktree 完全隔离。共享项（`decisions.md`、`patterns.md`、`index.md`、`domains/`、`project/`、`requirements/`、`active`、`autopilot.local.md` 等）以 per-item 符号链接指向主仓库 `.autopilot/<item>`。主仓库自身 session 仍写在 `.autopilot/active` / `.autopilot/requirements/`。
> **需求管理**：每次 autopilot 运行自动创建 requirements 文件夹，所有产出物归档其中。`task_dir` frontmatter 字段指向该文件夹。
...
|------|----------|-------------------|
| design | AskUserQuestion 审批 | 跳过审批，直接写设计文档 + plan-reviewer 审查 → 通过则推进 |
...
### 目标
完成设计文档并获得用户审批后进入实现阶段。设计文档直接写入状态文件。

### ⚠️ 关键规则
**进入 design 阶段后，按以下优先级决定设计模式**：
1. `auto_approve: true` → 走 [Auto-Approve 快速路径](#auto-approve-快速路径仅-auto_approvetrue-时)（跳过审批）
2. `fast_mode: true` → 走 [Fast Mode 快速路径](#fast-mode-快速路径仅-fast_modetrue-时)（跳过 brainstorm，1 个 Explore agent，编排器自审）
3. 其他（默认）→ 走 [Standard Design 模式（默认，含 brainstorm）](#standard-design-模式默认含-brainstorm)

### Standard Design 模式（默认，含 brainstorm）

默认触发，`--fast` 跳过。执行含 brainstorm 的交互式需求探索。此模式适用于所有常规任务，确保在编写设计文档前充分理解用户意图。

单流程：知识加载 → Explore agent → 视觉伴侣征求 → 逐个问问题（AskUserQuestion 逐个澄清）→ 方案选择 → 写设计文档到状态文件 → Plan Reviewer + Spec Reviewer → AskUserQuestion 审批

Q&A 结果写入 `$TASK_DIR/brainstorm.md`（`task_dir` 从 frontmatter 读取）。详细工作流参见 [brainstorm-guide.md](references/brainstorm-guide.md)。视觉伴侣使用说明参见 [visual-companion-guide.md](references/visual-companion-guide.md)。

兼容性说明：历史 state.md 中的 `plan_mode: "deep"` 同样走此分支（行为等同默认），`plan_mode` 字段已弃用，新代码不读。

### Fast Mode 快速路径（仅 fast_mode=true 时）

`fast_mode: true` 时**跳过 brainstorm Q&A** + 砍掉 scenario-generator 和 plan-reviewer 两个 Agent：知识加载 → **1 个** Explore agent（**不启动 scenario-generator**，**不做 brainstorm 交互探索**）→ 将设计文档写入状态文件 → 编排器按 `references/plan-reviewer-prompt.md` 中**全部维度自审**（**不启动 plan-reviewer Agent**）→ AskUserQuestion 审批。自审失败可修正后重审一次，仍失败则标注问题交用户判断。

### Auto-Approve 快速路径（仅 auto_approve=true 时）

当 `auto_approve` 为 `true` 时（自动链接的项目子任务），跳过审批：
...
6. **FAIL** → 设 `auto_approve: false`，回退到正常审批流程（步骤 1）
...
**默认前置**：先按 [brainstorm-guide.md](references/brainstorm-guide.md) 走 Q&A 探索（`fast_mode: true` 时跳过，直接进入代码探索）。
...
按 `html_review` 开关分流（优先级：frontmatter `html_review: true` > 环境变量 `AUTOPILOT_HTML_REVIEW=1` > 默认 false）：

- **关闭（默认）** → AskUserQuestion + preview 三选项（通过 / 修改 / 放弃），「通过」选项 preview 末尾追加 HTML 评审启用 hint
- **开启** → **前台同步**调 `bash ${CLAUDE_PLUGIN_ROOT}/scripts/visual-companion/launch-plan-review.sh "$task_dir"`（Bash 工具 `timeout: 600000`，**禁用 `run_in_background`**），解析 stdout JSON 的 `choice` 字段（approve/revise/abort），以 stdout 非空且合法 JSON 为成功判据；超时/失败 fallback 到默认路径

详细工作流（preview 文案、JSON 解析、降级、超时处理）参见 [references/html-review-guide.md](references/html-review-guide.md)。
...
**触发条件**：读取状态文件 frontmatter `contract_required` 字段。

- 字段缺失或值为 `false` → **跳过本步骤**，直接进入 Phase: qa
- 值为 `true` → 启动 contract-checker Agent 执行字面校验
...
完整 frontmatter 字段说明（包含 fast_mode: false 默认值、qa_scope 取值范围等）参见 [references/state-file-guide.md](references/state-file-guide.md)。AI 可写字段：`phase` / `gate` / `retry_count` / `mode` / `qa_scope` / `next_task` / `knowledge_extracted`。AI 不动字段：`iteration` / `max_iterations` / `max_retries` / `session_id` / `started_at` / `task_dir` / `fast_mode`（fast_mode 由 setup.sh 设置、stop-hook 降级，AI 只读不写）。`auto_approve` 由 stop-hook 设置。
