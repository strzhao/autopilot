...
### [2026-09-09] pending 检测双盲区根治（v3.70.1）——「修复方向正确但验证证据全落在掩蔽区」的回归如何漏网
<!-- tags: autopilot, stop-hook, has-pending-subagents, wc-leading-space, window-blind, sidechain-inflation, full-transcript-scan, fromjson-per-line, false-negative, reinject-loop, v3.66.0-regression, v3.70.1 -->
**Scenario**: v3.66.0 条件化丢首行上线后首个 >4MB 生产会话（martin）死循环：异步 Agent 在跑却被判无 pending → §9 重注入（iteration 烧到 21，用户手动停止多次）。两个盲区叠加：(D1) macOS BSD `wc -c <` 输出带前导空格 → `[[ =~ ^[0-9]+$ ]]` 恒假 → file_size 恒 0 → 丢首行在生产从未触发 → >4MB 时 jq 必 parse error → 集合判定全死退 fail-safe；(D2) 4MB 窗口自膨胀——还在跑的 sidechain 持续追加内容把 22 个 launch 标记全部推出窗外、已完成任务的完成通知反在窗内 → 即使 jq 健康也判 pending=∅。对照证据：26MB 会话（87f0c07a）窗口内有 4 个标记未触发；本次窗口内 0 个。红队 P1/P2 契约测试进一步暴露第三层：单条损坏行（如并发写入的撕裂末行）也能拖垮整条 jq 精确路径，退化到窗口限定弱 fail-safe——D1 的类属缺陷，实现期没问到，红队问到。
**Choice**: jq 集合计算改 `timeout 10 jq -Rs 'split("\n") | map(fromjson?)' "$transcript"` **全量直读**——窗口盲区随输入源切换消除、无字节截断故半截行整类不再产生、fromjson? 逐行容错使坏行只跳过不拖垮精确路径；fail-safe（末 4MB grep -a 计数）仅作最后防线；删条件化丢首行整段死代码。红蓝收敛方向为实现服从红队契约测试，测试零修改。
**Alternatives rejected**: 修 wc -c 空格保留窗口+条件化丢行（治标：窗口盲区 D2 仍在）；扩大窗口到全文但保留 -s 严格解析（单坏行仍拖垮整路）。
**Lesson**: (1) 回归验证的证据必须覆盖「机制被真实触发」的场景——v3.66.0 全部测试证据（C10a/C11a、真实 3.6MB transcript）都在 <4MB 或标记可见区，条件化逻辑在 macOS 生产从未执行过（恒假短路），绿灯全绿但机制是死的；「验证了修复」≠「验证了修复路径被执行」。(2) 「窗口 + 集合差」型检测对**活动追加的流式文件**天然有盲区：集合两半（启动标记/完成通知）以不同速率被推出窗口，差值无语义；流式 append-only 文件的集合判定必须全量输入。(3) 修复时用「这条链上还有没有同样失效模式」反问（[[2026-05-26] 单一根因偏见]）——本次 D1 的类属缺陷（单坏行→整路失效）就是实现期没问到、红队契约测试暴露的。
**Evidence**: 真实 6.9MB transcript 修复前 rc=1 假阴性、修复后独立 jq 真值 {sync:0, async:1, bash:0} 检出 rc=0；27MB 解析实测 0.2s（timeout 10 兜底）；红队 has-pending-full-scan 7 断言 + 既有 24 断言全绿；核对锚点：2026-09-09 stop-hook.sh has_pending_subagents（v3.70.1）。

### [2026-09-25] TaskStop 终止信号缺失（v3.73.1）——「终止信号只有一种」使假阳性永久固化且不随新内容自愈
<!-- tags: autopilot, stop-hook, has-pending-subagents, taskstop, background-task, silent-wait, false-positive-persistent, terminal-signal-union, fail-safe-over-subtract, v3.73.1 -->
**Scenario**: raven-cli 任务 `20260924-走-autopilot-实现这个方` 卡在 phase=implement / iteration 1：design-auto 已放行且 auto_approve=true，但此后每次 Stop 都落 §7.5「等待后台 sub-agent」分支——只发一句 ⏳ systemMessage 就 exit 0，循环整条死。根因：该 session 的 transcript 里有 2 个后台 Bash 任务（`bl59o8ybf`/`bgx5wsbzc`，2026-09-20 07:54/07:57 因 180s 超时转后台）被 **TaskStop** 显式停掉，而 TaskStop **不产生 `<task-id>` 完成通知**（harness 只回一条 tool_result `{"message":"Successfully stopped task: <id> (…)"}`）⇒ 路径 C 的「启动集 − 完成集」差集恒非空 ⇒ 假阳性**永久固化**（同一 transcript 上每次 Stop 都命中，换 session 续跑也不自愈）。全库实测（CC 2.1.270，1017 次 TaskStop 调用）：被显式停掉的 137 个任务只有 17 个收到过通知；近 21 天 **31/55** 个 session 已存在悬挂 id（在这些 session 里跑 autopilot 从一开始就是死的）。受控实验：同一 transcript + 同一 state，只补 2 条「已停止」通知 → 立刻从 ⏳ 变成正常 `decision: block`（iteration 1→2）。
**Choice**: 终止集从单一「完成通知」扩为**并集 = 完成通知 ∪ TaskStop 成功停止**（字面量 `Successfully stopped task: <id>`），路径 B（异步 Agent）与路径 C（后台 Bash）一并扣减；jq 精确路径与 jq 失败时的 grep fail-safe 文本路径同源修改；fail-safe 的计数对账只扣「停止 id ∩ 异步启动 id」的交集（`launched_count` 只统计 async_launched，全量扣减会过度抵消、把 fail-safe 从「宁判 pending」推向漏判）。
**Alternatives rejected**: (1) 只改 jq 路径不改 fail-safe——同一类静默卡死会在 jq 降级时换个入口复现；(2) 把失败文案「Task not found」（不含 id）也算终止——需按 tool_use_id 关联 TaskStop 的 input.task_id，实现面扩大，且当前 10/1017 次失败无一产生悬挂样本，暂不做（保守朝 pending）；(3) 给 §7.5 加 stale 逃生阀（同一 pending 集合连续 N 次不变即放行）——能兜底但会掩盖真信号，先做最小修复。
**Lesson**: (1) 「集合差」型检测的终止信号必须**穷举完备**：只要有一条终止路径不产出被识别的标记，该 id 就永久悬挂、假阳性不可自愈——而它的症状（循环不动）与「真的还在跑」完全同形，用户只看到一句看起来正常的等待提示。(2) 因果闭环靠**受控实验**：同一输入只改「终止信号」一处看行为是否翻转——A（原样）→ ⏳/iter 1、B（补通知）→ block/iter 2、C（改 hook 不改 transcript）→ block/iter 2 三点定位，比读代码推断可靠。(3) 排查入口可复用：`jq -Rs 'split("\n")|map(fromjson?)'` 算三路 pending 集合，对任意 session 的 transcript 一跑即知是否已中毒（同一函数上的第三次独立盲区，另见 [[2026-09-09] pending 检测双盲区根治]）。
**Evidence**: 真实 transcript（14MB / 7259 行）修复前 `pending=2` rc=0、修复后 rc=1；真值对照（造一个未闭合启动）仍 rc=0；端到端复演修复后源码 + 真实 state → `decision: block`、iteration 1→2；新增 `taskstop-terminal-signal.acceptance.test.mjs` 13 断言（含 jq shim 强制 fail-safe 路径 4 条）纳入 `npm test`，变异检验未修复版 8 失败 / 修复后全绿；`npm test` 93 测全绿、shellcheck 无告警。核对锚点：2026-09-25 stop-hook.sh has_pending_subagents（v3.73.1）。
