...
### [2026-07-19] 减法/重构任务系统性复刻 [2026-05-25]——claude-p 独立验证 + 既有断言语义化适配 + 补双重 grep
<!-- tags: autopilot, skill, shrink, refactor, claude-p, independent-verification, double-grep, assertion-adaptation, predicate-coverage, scene-5, 2026-05-25, dogfood, borderline-rollback, v3.58.1 -->
**Scenario**: autopilot/doctor SKILL.md 减法（582→510 / 664→570，删冗余散文 + 表格化 + 段落合并 + 章节合并），run-all 36/36 + 红队 10 断言全绿。但用户警示"减法曾致 autopilot 完全失效"，对照 [[2026-05-25]] cdad541 事故（删 merge §2 step 潜伏 27 天）发现本次同样动 step 文本，而既有 acceptance 只查契约词 grep（弱条件）+ 行数 wc（不查 step 段落）——[[2026-05-25]] 盲区仍在。
**Choice**: 减法/重构任务验证三件套：
① **claude -p 独立 headless session 读 skill quote 关键 step**——补 [[2026-05-25]] 明示的"静态 grep 抓不到 AI 读了是否执行"边界（至少抓到"AI 能否读到 step"）。本次 5 个中性问题（merge 第1步/auto-fix 含义/Tier 3.5 时序/防合理化 H5/Phase 顺序）+ 3 个真实需求（doctor 诊断/standard 全流程/fast mode），独立确认关键 step 减法后仍可读。中性 prompt 要点：Read only / quote verbatim / NOT FOUND / 不含预期暗示词。
② **既有断言对减法任务系统性误报，改语义化**：predicate-coverage SC1.P1 `added==0`（为 references/ 改动设计）对 SKILL.md 减法误报（行内删 git 计 added）→ 改 `deleted>=added`（净非增）；词频 `after>=before` 对删冗余误报 → 改 `after>=1`（语义保留，守护完全删除=真弱化）；`^+## ==0` 对章节合并误报 → 改 net 章节非增。判据：断言表达「约束本质」（只减不增=净非增 / 不弱化=词仍在 / 不加章节=net 非增）而非「机械字面」（added==0 / 词频不变 / 零新增行）。
③ **补 [[2026-05-25]] 双重 grep 长效守护**（scene 5：awk 提取 Phase 段 + grep -F 字面查 step 标题+字段），防未来减法删 step 潜伏。grep -F 避免 markdown `**` 被正则误解析。
