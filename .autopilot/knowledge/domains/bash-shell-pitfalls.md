...
**复现 [2026-09-25]**：验收套件排查时又扫出 **36 处**（11 文件）：`plugins/autopilot/tests/acceptance/` 10 文件 29 处 + `tests/contract-protocol/structural.acceptance.sh` 7 处（该文件 [2026-05-30] 修过一轮，**漏了这 7 处**）+ `plugins/autopilot/scripts/setup.sh:200` —— 最后这处是**生产告警路径**（「未知的审批门: $GATE（合法 gate 值…）」），`set -uo pipefail` 下直接崩，而不是打印未知 gate 值。最典型的危害形态：`knowledge-context-fanout` scene 5.P4 断言真失败（版本号硬编码 `3.69.0` 已过时）时，fail 消息自身崩在 `$n，`，**把真正的失败原因吃掉**，只剩一句 unbound variable，排查方向被带偏。同轮修复：36 处全加 braces（逐行配对核验：仅花括号插入、零语义变化）+ scene 5.P4 改为从 plugin.json 动态读版本（回归 [2026-05-09] 已根治的 TARGET_VERSION 盲区）。**并落机械守卫**：`tests/bash-var-multibyte-boundary.acceptance.test.mjs`（已入 `npm test`）扫描全仓 `*.sh` 非注释行的 `$VAR`+非 ASCII 形态（排除 `\$` 转义），违规即 CI 红——同一陷阱三次复发证明约定层防不住，改机械执法。
