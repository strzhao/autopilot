...
### [2026-07-17] 独立 skill 调用必须有 task_dir fallback——否则产物落 v3.35 前旧路径污染入库
<!-- tags: autopilot, brainstorm, independent-skill, task-dir, fallback, layered-layout, knowledge-runtime, gitignore, pollution, dogfood, claude-code-buddy, latent-bug, conditional-vs-unconditional-migration, v3.56.1 -->
**Scenario**: dogfood claude-code-buddy 实证顶层 `.autopilot/requirements/` 累积 13 slug / 29 条 runtime 产物（brainstorm.md/state.md/visual HTML）污染入库。根因：`/autopilot:autopilot-brainstorm` 被**独立调用**（非主流程委托）时无 state.md 提供 task_dir，SKILL.md 只写「从 state.md frontmatter 读 task_dir」**无 fallback** → AI 据过期规约印象落到顶层 `.autopilot/requirements/`（v3.35 二级分层前旧路径）。setup.sh 条件迁移（decisions.md 存在 ∧ knowledge/ 不存在）在混合状态短路，僵尸永不清；CLAUDE.md 守卫只查 `runtime/` 进 git status，顶层 `requirements/` 不在 runtime/ 下 → 守卫不触发，长期未发现。
**Lesson**: ① **独立调用的 skill 必须为每个外部依赖显式定义 fallback**——brainstorm 依赖 task_dir 但独立调用无 state.md，缺 fallback → AI 猜路径落旧规约。fallback 要给**明确目标路径 + slug 约定 + 禁止项 + 后果**（"严禁顶层，落那里等于丢失产物"），不只"别落错"。② **「目录名即答案」分层 SSOT 对第三类目录失效**——knowledge/runtime 二分只覆盖两类；新增子系统引入新目录（project/ / 顶层 requirements/ 等）必须同步纳入分层规约（CLAUDE.md 表格 + .gitignore），否则判定对它失效。③ **条件迁移治不了"今天还在写"**——setup.sh 迁移只在"整体旧布局"触发，混合状态（已建 knowledge/）短路。**无条件迁移**（每次 session 检查搬走）才是治持续泄漏的正解；条件迁移只治一次性历史遗留。④ **守卫范围 = 规约范围**——新增"顶层 requirements 也 ignore"后判定原则要补：任何 `.autopilot/` 下非 knowledge/ 产物进 git status = 异常。
